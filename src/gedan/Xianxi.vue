    ...
    <header  ref='header'>
     ...
     <div style="padding-top:5em;">
        ...
        <div style="width:60vw;margin-left:5vw;">
         <h4 style="color:white">{{listHeader.name}}</h4>
         ...
         <p style="margin-top:10px;color:gray; width:90%; font-size:12px;overflow: hidden;
      ...
      <table style="background:white; width:100vw; border-top-left-radius:20px;border-top-right-radius:20px;margin-top:10px;">
        <tr >
          ...
          <td style="" colspan="2" >
            <p style="background:red;border-radius:20px;text-align:Center;color:white;padding:10px 0px;margin-right:20px;font-size:12px;">+ 收藏({{listHeader.subscribedCount =  listHeader.subscribedCount / 10000 >= 0 ? Math.floor(( listHeader.subscribedCount / 10000)) +'万' :  listHeader.subscribedCount}})</p>
  ...
  methods: {
    ...
    handleScroll () {
      ...
      if(scrollTop >= 80 && scrollTop < 230) {
        ...
        this.$refs.header.style.background = 'pink';
      }else {
        this.title = '歌单'
    ...
    pusha(item) {
      ...
      if(cunzai != -1 && cunzai != 0) {
       this.$store.commit('unshiftA', this.$store.state.dai_musicList.splice(cunzai,1)[0])
